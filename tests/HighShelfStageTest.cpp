...
double warmupSine(HighShelfStage& filter, double frequency, double sampleRate,
...
class HighShelfStageTest : public ::testing::Test {
  protected:
    void SetUp() override {
        filter_ = std::make_unique<HighShelfStage>();
        filter_->prepareToPlay(kSampleRate, kBlockSize, kNumChannels);
    }

    std::unique_ptr<HighShelfStage> filter_;
};

// (1) Lifecycle: construct → prepare → process zero buffer doesn't crash and
// produces exact zeros.
TEST_F(HighShelfStageTest, ConstructionAndPrepareDoNotCrash) {
...
TEST_F(HighShelfStageTest, DisabledIsBitExactPassthrough) {
...
TEST_F(HighShelfStageTest, UnityGainIsTransparent) {
    filter_->setCutoffFrequency(2000.0);
...
// (4) Boost above cutoff: at +6 dB high-shelf, a sine well above the corner
// should see ~+6 dB gain (linear ratio ~1.995).
TEST_F(HighShelfStageTest, BoostAboveCutoff) {
    constexpr double cutoff = 1000.0;
    constexpr double gainDb = 6.0;
    constexpr double frequency = 4000.0; // well above cutoff
    ...
    EXPECT_NEAR(ratio, expected, 0.1)
        << "4 kHz at +6 dB high-shelf (1 kHz corner) should boost by ~6 dB. Got ratio " << ratio;
}

// (5) No effect below cutoff: at +12 dB high-shelf, a sine well BELOW the
// corner should be approximately unchanged.
TEST_F(HighShelfStageTest, NoEffectBelowCutoff) {
    constexpr double cutoff = 4000.0;
    constexpr double gainDb = 12.0;
    constexpr double frequency = 100.0; // far below cutoff
    ...
    EXPECT_NEAR(ratio, 1.0, 0.05)
        << "100 Hz with high-shelf at 4 kHz / +12 dB should be nearly unchanged. Got ratio " << ratio;
}

// (6) Cut symmetric to boost: -6 dB should attenuate by 1/1.995 ~= 0.501.
TEST_F(HighShelfStageTest, CutSymmetricToBoost) {
    constexpr double cutoff = 1000.0;
    constexpr double gainDb = -6.0;
    constexpr double frequency = 4000.0; // well above cutoff
    constexpr double amplitude = 0.5;
    ...
    EXPECT_NEAR(ratio, expected, 0.05)
        << "4 kHz at -6 dB high-shelf (1 kHz corner) should cut by ~6 dB. Got ratio " << ratio;
}

// (7) Cutoff setter clamps to [20, 20000].
TEST_F(HighShelfStageTest, CutoffSetterClamps) {
...
TEST_F(HighShelfStageTest, GainSetterClamps) {
    filter_->setGainDb(6.0);
...
TEST_F(HighShelfStageTest, ResetClearsState) {
    filter_->setCutoffFrequency(2000.0);
    filter_->setGainDb(12.0);

    auto impulseTail = [&](HighShelfStage& f) {
    ...
    filter_->setCutoffFrequency(2000.0);
    filter_->setGainDb(12.0);
    warmupSine(*filter_, 2000.0, kSampleRate, 0.5, 1, 5000);
    filter_->reset();
...
TEST_F(HighShelfStageTest, MultiChannelIdenticalForIdenticalInput) {
    filter_->setCutoffFrequency(2000.0);
    filter_->setGainDb(6.0);
...
TEST_F(HighShelfStageTest, StabilityUnderSweep) {
    filter_->setGainDb(12.0);

    juce::Random rng(0xBD512AB3u);
