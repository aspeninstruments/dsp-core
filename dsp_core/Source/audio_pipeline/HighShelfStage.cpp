...
void HighShelfStage::writeCoefficients(double cutoffHz, double gainDb) {
    // RBJ high-shelf biquad computed in place to match
    // juce::dsp::IIR::Coefficients<double>::makeHighShelf exactly (Butterworth Q).
    const double gainLinear = juce::Decibels::decibelsToGain(gainDb);
    ...
    const double beta = std::sin(omega) * std::sqrt(A) / kHighShelfButterworthQ;
